...
const FONT_SIZE_STEPS = [36, 44, 52, 60, 68, 76, 84];

export function ExpressionFontSizeScreen() {
  const settings = useCalculatorStore((s) => s.settings);
  const updateSettings = useCalculatorStore((s) => s.updateSettings);

  const activeSize = settings.expressionFontSize;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.previewContainer}>
        <View style={styles.previewList}>
          {mockExprs.map((val, idx) => (
            <View key={idx} style={styles.previewRow}>
              <Text
                style={[
                  styles.previewText,
                  { fontSize: activeSize, color: '#FFFFFF', fontWeight: '500' },
                ]}
                numberOfLines={2}
              >
                {val}
          ...
          <View style={styles.ticksContainer}>
            {FONT_SIZE_STEPS.map((size) => {
              ...
              return (
                <Pressable
                  key={size}
                  onPress={() => updateSettings({ expressionFontSize: size })}
                  style={styles.tickPressable}
                  accessibilityRole="button"
                  accessibilityLabel={`Set expression font size to ${size}`}
                >
                  {isActive ? (
